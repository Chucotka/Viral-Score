...
/** Paid access still active (null accessExpiresAt = lifetime / legacy unlock). */
export function isPremiumAccessActive(client) {
  if (!client?.accessUnlocked) return false;
  const exp = client.accessExpiresAt;
  if (!exp) return true;
  return new Date(exp).getTime() > Date.now();
}

export function hasPremiumAccess(client, clientId) {
  return isDeveloperClient(clientId) || isPremiumAccessActive(client);
}
